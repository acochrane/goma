  ...
  for (i = 0; i < upd->Num_Mat; i++)
    {
     ...
     ddd_add_member(n, mp_glob[i]->d_PorousLiqRefPress,
      ...
      /*
       * Couple two-phase membrane parameters
       */
      ddd_add_member(n, &mp_glob[i]->ctpm_def_roll_model, 1, MPI_INT);
      ddd_add_member(n, &mp_glob[i]->len_ctpm_def_roll_const, 1, MPI_INT);
  ...
  for (i = 0; i < upd->Num_Mat; i++)
    {
      ...
      dalloc( m->len_ctpm_def_roll_const,
              m->    ctpm_def_roll_const);
...
noahs_dove()
{
  ...
  for (i = 0; i < upd->Num_Mat; i++) {
    ...
    crdv( m->len_ctpm_def_roll_const,
	  m->    ctpm_def_roll_const);
