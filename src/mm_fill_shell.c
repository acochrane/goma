                     ...
                     const Exo_DB *exo )
{
  ...
  */
  ...
  if(mp->ctpm_def_roll_model != NO_MODEL) {
    //      memset (dProll_dh, 0.0, sizeof(double)*MDE);
    memset (dProll_dx, 0.0, sizeof(double)*MDE);
  }
  ...
  */
  ...
  if (pd->e[R_TFMP_MASS] || mp->ctpm_def_roll_model != NO_MODEL) {
  ...
  //contribution from roller pressure
  switch(mp->ctpm_def_roll_model) {
  case CONSTANT_SPEED:
    Proll = rolling_pressure(h, &xr, time);
    dProll_dh = mp->ctpm_def_roll_const[3];

    for (j=0; j<ei->dof[MESH_DISPLACEMENT1]; j++) {
      dProll_dx[j] += mp->ctpm_def_roll_const[3] // k
	*-0.5
	*1.0/sqrt(mp->ctpm_def_roll_const[4]*mp->ctpm_def_roll_const[4] // R^2
		  - (fv->x[0] - xr)*(fv->x[0] - xr)
		  )
	*2.0*(fv->x[0] - xr)*bf[MESH_DISPLACEMENT1]->phi[j];
    }
    break;
  default:
    Proll = 0.0;
  ...
  if (Proll>0) {
    ...
    for (j=0; j<ei->dof[var]; j++) {
      for (k = 0; k<DIM; k++) {
	dP_load_dmesh[k][j] += -dProll_dh*dh_dmesh[k][j];
      }
      dP_load_dmesh[0][j] += -dProll_dx[j]; // d{fv->x[0]}_dmesh = phi_j;
    }
    var = SHELL_NORMAL1;
    for (j=0; j<ei->dof[var]; j++) {
      for (k = 0; k<DIM; k++) {
	dP_load_dnormal[k][j] += -dProll_dh*dh_dnormal[k][j];
      }
    }
  }
