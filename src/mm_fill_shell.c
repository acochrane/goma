 ...
 *                                   set or tfmp_sat field for a lubrication field.
 *
 * Created:     October 19, 2010 sarober@sandia.gov
 * Hijacked:		May         2015 acochrane@gmail.com for use with assemble_shell_tfmp
 */
int
assemble_lubrication_curvature(
			       double time,              /* present time value */ 
  ...
  /* Bail out fast if there's nothing to do */
  if ( !pd->e[eqn]  ) return(status);
  if ( !(pd->e[FILL] || pd->e[R_TFMP_BOUND]) ) EH(-1, "Must activate level set or thin-film multi-phase equation to calculate curvature.");
  ...
  dbl LSnormal[DIM], LSnormal_mag = 0.0, LSnormal_maginv;
  for ( i = 0; i < DIM; i++ ) {
  ...
  for ( i = 0; i < ei->dof[adv_var]; i++ ) {

    /* Basis functions */
    ShellBF( adv_var, i, &phi_i, grad_phi_i, grad_II_phi_i, d_grad_II_phi_i_dmesh, n_dof[MESH_DISPLACEMENT1], dof_map );
 ...
 *                      thin-film multi-phase flow. Two-phase would be more accurate
 *                      
 *
 * Currently, the following equations are solved.  This is likely to change as
 * different things are tested.  "Use the source, Luke".
 *
 *    0 = d/dt(h*rho) + (v*rho)dot(gradII(h)) + h*divII(v*rho)
 *    0 = d/dt(S) + S/h*(v)dot(gradII(h)) + (SU/PG){(v)dot(gradII(S)}
 *
 * where rho and mu are determined by hyperbolic tangent functions of S
 ...
 *         v = velocity_aux as solved in the porous term of assemble_momentum
 * Input
 * =====
 * time_value = The current time.
 * theta      = The implicit-explicit time stepping parameter.
 ...
 *    *Note: Much was borrowed from assemble_lubrication
...
/* Use the height_function_model */
  ...
  if (pd->e[R_MOMENTUM1]) { // VPS Formulation
    ...
    if (mp->tfmp_wt_model != GALERKIN) {
      for (k=0; k<DIM; k++) {
	(pg_data->gp_gradII_P[k]) = gradII_P[k];
      }
      pg_data->gp_h = h;
      pg_data->gp_mu = mu;
  ...
  if(mp->tfmp_wt_model != GALERKIN) {
    tfmp_PG_gausspt(pg_data);
  }

  /* allocate for various dot products */
  double gradS_dot_gradphi_i, gradphi_i_dot_gradphi_j, gradS_dot_gradphi_j;
  double gradS_dot_gradP, gradP_dot_gradphi_i, gradP_dot_gradphi_j;
  double v_dot_grad_phi_i, v_dot_grad_phi_j, v_dot_grad_h, v_dot_grad_S;
  double dv_cent_dvj, dh_elem_dvj;
  v_dot_grad_h = 0.0;
  v_dot_grad_S = 0.0;
  gradS_dot_gradP = 0.0;
  for (k=0;k<DIM;k++) {
    ...
    gradS_dot_gradP += gradII_S[k]*gradII_P[k];
  }
  ...
  if ( af->Assemble_Residual ) {
	...
	if ( T_ADVECTION ) {
	  adv += pg_data->wt_func*v_dot_grad_S;
  ...
  if (af->Assemble_Jacobian) {
	...
	for ( j = 0; j < ei->dof[var]; j++) {
	  ...
	  if ( T_DIFFUSION ) {
	    ...
	    diff += h*h*h/12.0/mu*gradphi_i_dot_gradphi_j;
	  }
	  diff *= etm_diff_eqn;
	  ...
	  if ( T_DIFFUSION ) {
	    ...
	    diff += h*h*h/12.0*gradP_dot_gradphi_i*(-1./mu/mu*dmu_dS*phi_j);
	    diff *= etm_diff_eqn;
	  }
	  // Assemble full Jacobian
	  lec->J[peqn][pvar][i][j] += dA*(mass + adv + diff);
 ...
 */
  ...
  if (af->Assemble_Jacobian) {
    ...
    etm_diff_eqn = pd->etm[eqn][(LOG2_DIFFUSION)];
    // Loop over DOF (i)
    for ( i = 0; i < ei->dof[eqn]; i++) { /* The sensitivities of R_TFMP_BOUND to VELOCITY and TFMP_SAT */
      // Load basis functions
      ShellBF( eqn, i, &phi_i, grad_phi_i, gradII_phi_i, d_gradII_phi_i_dmesh, n_dof[MESH_DISPLACEMENT1], dof_map );
      tfmp_PG_dof(&phi_i, gradII_phi_i, pg_data);
	...
	for (l = 0; l<DIM;l++) {
	  ...
	  if (pd->v[var]) {
	    ...
	    for ( j = 0; j < ei->dof[var]; j++) {
	      ...
	      ShellBF( var, j, &phi_j, grad_phi_j, gradII_phi_j, d_gradII_phi_j_dmesh, n_dof[MESH_DISPLACEMENT1], dof_map );
	      if(mp->tfmp_wt_model != GALERKIN) {
		tfmp_PG_dvarj(&phi_i, gradII_phi_i, j, &phi_j, gradII_phi_j, pg_data, l);
	      }
	      // Assemble mass term 
	      mass = 0.0;
	      if ( T_MASS ) {
		mass += pg_data->dwt_func_dvarj[l] * fv_dot->tfmp_sat;
		mass *= etm_mass_eqn;
	      }
	      // Assemble advection term
	      adv = 0.0;
		...
		// gradII_S component
		adv += pg_data->wt_func*phi_j*gradII_S[l];
		adv += pg_data->dwt_func_dvarj[l]*v_dot_grad_S;
	...
	if (pd->v[var]) {
	  ...
	  for ( j = 0; j < ei->dof[var]; j++) {
	    ...
	    mass = 0.0;
	    if( T_MASS ) {
	      mass += pg_data->wt_func*phi_j*((1+2*tt)/delta_t);
	      mass += pg_data->dwt_func_dvarj[4]*fv_dot->tfmp_sat;
	    }
	    mass *= etm_mass_eqn;
	    // Assemble advection term
	    adv = 0.0;
		...
		v_dot_grad_phi_j += vII[k]*gradII_phi_j[k];
	      ...
	      adv += pg_data->wt_func*v_dot_grad_phi_j;
	      adv += pg_data->dwt_func_dvarj[4]*v_dot_grad_S;
	      adv *= etm_adv_eqn;
	    }
	    // Assemble diffusion term
	    diff = 0.0;
      ...
      } // End of VPS Formulation Sensitivites of R_TFMP_BOUND
      else { // PS formulation    
	var = TFMP_PRES;
	if (pd->v[var]) {
	  pvar = upd->vp[var];
	  ...
	  for ( j = 0; j < ei->dof[var]; j++) {
	    ...
	    mass = 0.0;
	    if( T_MASS ) {
	      mass += pg_data->dwt_func_dvarj[3]*fv_dot->tfmp_sat;
	    }
...
/*
      ...
      } // End of PS Formulation Sensitivites of R_TFMP_BOUND
    } // End of loop over DOF (i)
                 ...
                 const Exo_DB *exo)
{
  ...
  dbl phi_j, grad_phi_j[DIM], gradII_phi_j[DIM]; // Basis funcitons (j)
  dbl d_gradII_phi_j_dmesh[DIM][DIM][MDE];
  dbl gradP_dot_gradphi_i;
  dbl gradphi_i_dot_gradphi_j;
  dbl mass, adv, adv1, diff; // Residual terms
  dbl etm_mass_eqn, etm_adv_eqn, etm_diff_eqn;
  int eqn;
  dbl supg = 0.;
  ...
  if ( af->Assemble_Residual ) {
	...
	adv += h*h*h/12.0/mu*gradP_dot_gradphi_i;
  ...
  if (af->Assemble_Jacobian) {
    ...
    etm_diff_eqn = pd->etm[eqn][(LOG2_DIFFUSION)];
    // Loop over DOF (i)
    for ( i = 0; i < ei->dof[eqn]; i++) { /* The sensitivity of R_TFMP_MASS */
      // Load basis functions
	...
	for ( j = 0; j < ei->dof[var]; j++) {
	  ...
	  if ( T_ADVECTION ) {
	    ...
	    adv += h*h*h/12.0/mu*gradphi_i_dot_gradphi_j;
