...
/*added by DSB 7/13; some post proc routines for 2D/shell problems
  ...
  /* features of the coupled tfmp + shell_mesh equations */

  if(pd_glob[mn]->e[R_TFMP_MASS] && pd_glob[mn]->e[R_MESH1]) {
    char input[MAX_CHAR_IN_INPUT] = "zilch\0";
    strcpy(search_string, "Deformable Roller");
    model_read = look_for_optional(imp,
				   search_string,
				   input,
				   '=');
	...
	EH(-1, err_msg);
      ...
      SPF(es, "%s = %s", search_string, model_name);
      if (!strcmp(model_name, "CONSTANT_SPEED") ) {
	mat_ptr->ctpm_def_roll_model = CONSTANT_SPEED;
	num_const = read_constants(imp, &(mat_ptr->ctpm_def_roll_const), NO_SPECIES);
	if (num_const != 5) {
	  EH(-1,
	     "Fixed Deformable Roller 'CONSTANT_SPEED' model requires five input values, Initial X offset, Roller axis velocity, the separation between the roller axis and the template, the spring constant of the roller and the roller radius");
	}
	mat_ptr->len_ctpm_def_roll_const = num_const;
	SPF_DBL_VEC( endofstring(es), num_const, mat_ptr->ctpm_def_roll_const );
      }
    } else {
      mat_ptr->ctpm_def_roll_model = NO_MODEL;
    }
    ECHO(es, echo_file);
