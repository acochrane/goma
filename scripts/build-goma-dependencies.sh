...
#matio
if [ -d matio-1.5.2/matio ]
then
    echo "matio already built"
else
    cd matio-1.5.2
    ./configure --with-hdf5=$GOMA_LIB/hdf5-1.8.12/hdf5 --prefix=$GOMA_LIB/matio-1.5.2/matio --enable-shared=off
    make -j$MAKE_JOBS
    make install
    cd ..
fi
...
#netcdf
if [ -d netcdf-4.3.2/netcdf ]
then
    echo "netcdf already built"
else
    echo 
    cd $GOMA_LIB/netcdf-4.3.2
    cd include
    echo "$NETCDF_PATCH" > netcdf.patch
    patch -f --ignore-whitespace netcdf.h < netcdf.patch
    cd ..
    export CPPFLAGS=-I$GOMA_LIB/hdf5-1.8.12/hdf5/include
    export LDFLAGS=-L$GOMA_LIB/hdf5-1.8.12/hdf5/lib 
    ./configure --prefix=$GOMA_LIB/netcdf-4.3.2/netcdf --enable-shared=off
    make -j$MAKE_JOBS
    make install
    cd ..
fi
...
SEACAS_LIBS="-L${GOMA_LIB}/hdf5-1.8.12/hdf5/lib -lhdf5_hl -lhdf5 -lz -lm"
# Install directory
...
-D Trilinos_ENABLE_SECONDARY_STABLE_CODE:BOOL=ON \
      -D Netcdf_LIBRARY_DIRS:PATH="$GOMA_LIB/netcdf-4.3.2/netcdf/lib" \
      -D TPL_ENABLE_Netcdf:BOOL=ON \
      -D TPL_Netcdf_INCLUDE_DIRS:PATH="$GOMA_LIB/netcdf-4.3.2/netcdf/include" \
      -D Matio_LIBRARY_DIRS:PATH=$GOMA_LIB/matio-1.5.2/matio/lib \
      -D Matio_INCLUDE_DIRS:PATH=$GOMA_LIB/matio-1.5.2/matio/include \
...
-D Trilinos_EXTRA_LINK_FLAGS:STRING="-I$GOMA_LIB/SuiteSparse/SuiteSparse_config $FORTRAN_LIBS $SEACAS_LIBS $MPI_LIBS" \
